...
public class GeneralResourceRegister {
	...
	//Made this public to allow a test to check if the flies load
	public GeneralResourceRegister() {
	...
	private void loadNameLists() {
		try {
			InputStream firstNameFileLocation = getClass().getClassLoader().getResourceAsStream("/peon/firstnames.txt");
			InputStream lastNameFileLocation = getClass().getClassLoader().getResourceAsStream("/peon/lastnames.txt");

			System.err.println("first names location: " + firstNameFileLocation);

			BufferedReader firstNamesReader = new BufferedReader(new InputStreamReader(firstNameFileLocation));
			BufferedReader lastNamesReader = new BufferedReader(new InputStreamReader(lastNameFileLocation));
